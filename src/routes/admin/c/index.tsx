...
import { PageHeader, EmptyState, Card, CardHeader, CardTitle, CardDescription, Button } from '@/components/ui';
...
export const onRequestGet = factory.createHandlers(requireAuth(), async (c) => {
  ...
  const collections = (await listCollections(getDb(c.env.DB))).filter((col) => col.slug !== 'media');

  return c.render(
    <AdminShell user={user} current="content">
      <PageHeader title="Content" description="Browse and author documents across your collections." />
      {collections.length === 0 ? (
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {collections.map((col) => (
            <a href={`/admin/c/${col.slug}`} class="block rounded-lg transition-shadow hover:shadow-md">
              <Card>
                <CardHeader>
                  <CardTitle>{col.name}</CardTitle>
                  <CardDescription>
                    {col.shape === 'singleton' ? 'Singleton' : 'Collection'} · {col.fields.length} field
