    ...
    <AuthShell>
      <div class="flex flex-col gap-4 text-center">
        <p class="text-sm text-ink-muted">
          This invite link is invalid, revoked, used up, or has expired.
        </p>
  ...
  return c.render(
    <AuthShell>
      <form
        ...
        data-on:submit={`@post('/auth/join/${token}', {contentType: 'form'})`}
      >
        <p class="text-sm text-ink-muted">
          You've been invited to a team. Create your account to join.
        </p>
...
export const onRequestPost = factory.createHandlers(
  rateLimit('join', LOGIN_RATE_LIMIT),
  async (c) => {
    const token = c.req.param('token') ?? '';
    ...
    try {
      ...
      if (err instanceof InputValidationError)
        return c.html(inlineError(err.details?.[0]?.message ?? 'Invalid details.'));
